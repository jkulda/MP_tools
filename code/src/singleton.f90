  MODULE singleton
    ...
    FUNCTION fft3d(array, dim, inv) RESULT(ft)
      ...
      LOGICAL,                            INTENT(IN),  OPTIONAL:: inv
      !--- function result
      COMPLEX(fftkind),DIMENSION(SIZE(array, 1), SIZE(array, 2), SIZE(array, 3)):: ft
    ...
    LOGICAL,                              INTENT(IN),  OPTIONAL:: inv
    !--- function result
    COMPLEX(fftkind), DIMENSION(SIZE(array,1),SIZE(array,2),SIZE(array,3),SIZE(array,4)):: ft
    ...
    work = RESHAPE(array, (/ SIZE(array) /))
    CALL fftn(work, SHAPE(array), dim, inv, stat = status)
    ft = RESHAPE(work,(/SIZE(array,1),SIZE(array,2),SIZE(array,3),SIZE(array,4)/))
    DEALLOCATE( work )
    ...
    LOGICAL,                                INTENT(IN),  OPTIONAL:: inv
    !--- function result
    COMPLEX(fftkind),DIMENSION(SIZE(array,1),SIZE(array,2),SIZE(array,3),SIZE(array,4),SIZE(array,5)):: ft
    ...
    work = RESHAPE(array, (/ SIZE(array) /))
    CALL fftn(work, SHAPE(array), dim, inv, stat = status)
    ft = RESHAPE(work, (/ SIZE(array,1), SIZE(array,2), SIZE(array,3),SIZE(array,4), SIZE(array,5) /))
    DEALLOCATE( work )
    ...
    FUNCTION fft6d(a, dim, inv) RESULT(ft)
    !--- formal parameters
    COMPLEX(fftkind), DIMENSION(:,:,:,:,:,:), INTENT(IN)           :: a
    INTEGER,          DIMENSION(:),           INTENT(IN),  OPTIONAL:: dim
    LOGICAL,                                  INTENT(IN),  OPTIONAL:: inv
    !--- function result
    COMPLEX(fftkind),DIMENSION(SIZE(a,1),SIZE(a,2),SIZE(a,3),SIZE(a,4),SIZE(a,5),SIZE(a,6)):: ft
    ...
    CALL fftn(work,SHAPE(a),dim,inv,stat=status)
    ft = RESHAPE(work,(/SIZE(a,1),SIZE(a,2),SIZE(a,3),SIZE(a,4),SIZE(a,5),SIZE(a,6)/))
    DEALLOCATE(work)
    ...
    FUNCTION fft7d(a, dim, inv) RESULT(ft)
    !--- formal parameters
    COMPLEX(fftkind), DIMENSION(:,:,:,:,:,:,:), INTENT(IN)           :: a
    INTEGER,          DIMENSION(:),             INTENT(IN),  OPTIONAL:: dim
    LOGICAL,                                    INTENT(IN),  OPTIONAL:: inv
    !--- function result
    COMPLEX(fftkind),DIMENSION(SIZE(a,1),SIZE(a,2),SIZE(a,3),SIZE(a,4),SIZE(a,5),SIZE(a,6),SIZE(a,7)):: ft

    !--- Allocate 1-D a to be used by routine fftn
    COMPLEX(fftkind), DIMENSION(:), ALLOCATABLE :: work

    ALLOCATE( work(SIZE(a)) )
    work = RESHAPE(a,(/SIZE(a)/))
    CALL fftn(work, SHAPE(a), dim, inv, stat = status)
    ft = RESHAPE(work,(/SIZE(a,1),SIZE(a,2),SIZE(a,3),SIZE(a,4),SIZE(a,5),SIZE(a,6),SIZE(a,7)/))
    DEALLOCATE(work)
    ...
    DO i = 1, ntotal
       array(i) = CMPLX(REAL(array(i))*scale,AIMAG(array(i))*scale, KIND=fftkind)
    END DO

    DO i = 1, ndim
       CALL fftradix(array,ntotal,shape(d(i)),PRODUCT(shape(1:d(i))),inverse)
    END DO
    ...
    SUBROUTINE transform(array,ntotal,npass,nspan,factor,nfactor,inv) !-- compute fourier transform
    !--- formal parameters
    COMPLEX(fftkind), DIMENSION(:), INTENT(IN OUT):: array
    INTEGER,                        INTENT(IN)    :: ntotal, npass, nspan
      ...
      CASE (3) !-- transform for factor of 3 (optional code)
       DO
        DO
         ...
         array(kk) = ck + cj
         ck = ck-CMPLX(0.5_fftkind*REAL (cj),0.5_fftkind*AIMAG(cj),KIND=fftkind)
         cj = CMPLX((REAL(array(k1))-REAL(array(k2)))*s60,(AIMAG(array(k1))-AIMAG(array(k2)))*s60,KIND=fftkind)
      ...
      CASE (5) !-- transform for factor of 5 (optional code)
       ...
       DO
        DO
         ...
         cc = array(kk)
         array(kk) = cc + ckp + cjp
         ck = CMPLX(REAL(ckp)*c72,AIMAG(ckp)*c72,KIND=fftkind)+CMPLX(REAL(cjp)*c2,AIMAG(cjp)*c2,KIND=fftkind)+cc
         cj = CMPLX(REAL(ckm)*s72,AIMAG(ckm)*s72,KIND=fftkind)+CMPLX(REAL(cjm)*s2,AIMAG(cjm)*s2,KIND=fftkind)
         array(k1) = ck + CMPLX(-AIMAG(cj), REAL(cj), KIND=fftkind)
         array(k4) = ck + CMPLX(AIMAG(cj), -REAL(cj), KIND=fftkind)
         ck = CMPLX(REAL(ckp)*c2,AIMAG(ckp)*c2,KIND=fftkind)+CMPLX(REAL(cjp)*c72,AIMAG(cjp)*c72,KIND=fftkind)+cc
         cj = CMPLX(REAL(ckm)*s2,AIMAG(ckm)*s2,KIND=fftkind)-CMPLX(REAL(cjm)*s72,AIMAG(cjm)*s72,KIND=fftkind)
         array(k2) = ck + CMPLX(-AIMAG(cj), REAL(cj), KIND=fftkind)
    ...
    CASE default
     ...
     DO
        ...
        DO
           k = k+1
           ck = ck+CMPLX(REAL(ctmp(k))*cosine(jj),AIMAG(ctmp(k))*cosine(jj),KIND=fftkind)
           k = k + 1
           cj = cj + CMPLX(REAL(ctmp(k))*sine(jj),AIMAG(ctmp(k))*sine(jj), KIND=fftkind)
           jj = jj + j
           IF (jj > jf) jj = jj - jf
           IF (k >= jf) EXIT
        END DO
